<script lang="ts">
	...
	async function seatGuests() {
		seatingError = '';
		const res = await fetch(`${API_BASE}/table-groups`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` }, body: JSON.stringify({ table_ids: seatTargetIds, party_size: seatPartySize, name: seatName || undefined }) });
		if (!res.ok) { seatingError = 'Failed to seat'; return; }
		seating = false; selectedTable = null; seatName = ''; seatPartySize = 2; seatTargetIds = []; await load();
	}

	async function loadGroup(id: number) { const r = await fetch(`${API_BASE}/table-groups/${id}`, { headers: { Authorization: `Bearer ${token()}` } }); if (r.ok) selectedGroup = await r.json(); }
	async function closeGroup(id: number) { await fetch(`${API_BASE}/table-groups/${id}/close`, { method: 'POST', headers: { Authorization: `Bearer ${token()}` } }); selectedTable = null; selectedGroup = null; await load(); }
	async function mergeTables() {
		if (!selectedGroup) return;
		await fetch(`${API_BASE}/table-groups/${selectedGroup.id}/tables`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` }, body: JSON.stringify({ add_table_ids: mergeTargetIds, remove_table_ids: [] }) });
		merging = false; mergeTargetIds = []; await loadGroup(selectedGroup.id); await load();
	}
	async function splitTable(tid: number) {
		if (!selectedGroup) return;
		await fetch(`${API_BASE}/table-groups/${selectedGroup.id}/tables`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` }, body: JSON.stringify({ add_table_ids: [], remove_table_ids: [tid] }) });
		await loadGroup(selectedGroup.id); await load();
	}

	function handleTableClick(t: Table) {
		selectedTable = t;
		if (t.status === 'free') { seating = true; seatingError = ''; seatName = ''; seatPartySize = Math.min(2, t.capacity); seatTargetIds = [t.id]; }
		else { seating = false; if (t.group_id) loadGroup(t.group_id); }
	}
...
<div class="space-y-4">
	{#if notifications.length > 0}
		<div class="alert alert-success">
			<div class="font-semibold">Items ready! ({notifications.length})</div>
			{#each notifications.slice(0, 3) as n}
				<div class="text-sm">×{n.quantity} {n.dish_name} — Table {n.table_name} <a href="/waiter/orders/{n.order_id}" class="link">View Order</a></div>
			{/each}
		</div>
	{/if}
	...
	{:else}
		<div class="overflow-x-auto">
			<table class="table table-zebra">
				<thead><tr><th>Table</th><th>Capacity</th><th>Label</th><th>Status</th><th>Group</th><th>Est. Free</th></tr></thead>
				<tbody>
...
{#if seating && selectedTable}
	<div class="modal modal-open"><div class="modal-box"><h3 class="font-bold text-lg">Seat {selectedTable.name}</h3>
		...
		<div class="py-4 space-y-3">
			...
			<label class="form-control"><div class="label"><span class="label-text">Name</span></div><input type="text" bind:value={seatName} placeholder="Birthday party" class="input input-bordered" /></label>
		</div>
		<div class="modal-action"><button class="btn" onclick={() => { seating = false; selectedTable = null; }}>Cancel</button><button class="btn btn-primary" onclick={seatGuests}>Seat</button></div>
	</div></div>
{/if}

<!-- Group detail modal -->
{#if selectedGroup}
	<div class="modal modal-open"><div class="modal-box"><h3 class="font-bold text-lg">{selectedGroup.name || 'Table Group'}</h3>
		<div class="py-4 space-y-3">
			<p class="text-sm">Party of {selectedGroup.party_size} | Tables: {selectedGroup.table_ids?.map(id => tableName(id)).join(', ') || '—'}</p>
			<p class="text-sm text-base-content/50">Est. free: {estimatedFree(selectedGroup.table_ids?.[0] ?? 0) ? new Date(estimatedFree(selectedGroup.table_ids![0])!).toLocaleTimeString() : '—'}</p>
			...
			{/if}
			{#if merging}
				<div class="mt-2"><p class="text-sm font-semibold mb-1">Add free tables:</p>
					<div class="flex flex-wrap gap-2">
						{#each freeTables() as t}
							<button class="btn btn-outline btn-xs" class:btn-active={mergeTargetIds.includes(t.id)} onclick={() => { mergeTargetIds = mergeTargetIds.includes(t.id) ? mergeTargetIds.filter(id => id !== t.id) : [...mergeTargetIds, t.id]; }}>{t.name}</button>
						{/each}
					</div>
